...
class ChatModoApp {
    constructor() {
        ...
        this.scrollLatestBtn = document.getElementById('scroll-latest-btn');
        this.isUserAtBottom = true;
        this.newMessageWhileScrolledUp = false;
        this.container.addEventListener('scroll', () => this.handleScroll());
        this.scrollLatestBtn.addEventListener('click', () => this.scrollToBottom(true));
        this.hideScrollBtn();
    }
    ...
    initializeUI() {
        ...
        // Nouveau bouton pour la console
    ...
    showScrollBtn() {
        this.scrollLatestBtn.style.display = 'flex';
    }
    hideScrollBtn() {
        this.scrollLatestBtn.style.display = 'none';
    }
    ...
    /**
     * Parse a raw IRC message from Twitch and extract username, color, emotes, etc.
     * Passes the color to addMessage.
     */
    parseIRCMessage(rawMessage) {
        ...
        try {
            ...
            // Extraire la couleur Twitch depuis les tags IRC
            let userColor = null;
    ...
    initializeFilters() {
        // Recherche avancée : mot-clé et mention
    ...
    shouldShowMessage(messageData) {
        ...
        if (keywordInput && keywordInput.value.trim() !== '') {
            ...
            if (!messageData.text.toLowerCase().includes(keyword)) {
                return false;
            }
        }
        ...
        const mentionInput = document.getElementById('mention-search');
        if (mentionInput && mentionInput.value.trim() !== '') {
            let mention = mentionInput.value.trim();
            if (mention.startsWith('@')) mention = mention.slice(1);
            if (!messageData.text.toLowerCase().includes(mention.toLowerCase())) {
                return false;
            }
        }
        ...
        try {
            ...
            if (messageText.trim()) {
                this.addMessage({
                    ...
                    badges: badges,
                    twitchEmotes: twitchEmotes,
                    color: userColor // Passe la couleur Twitch ici
                });
            }
        } catch (error) {
            console.error('Erreur lors du parsing du message:', error);
    ...
    getUserColor(username, colorHex) {
        // If Twitch color is provided, use it directly
        if (colorHex) {
            return colorHex;
        }
        // Otherwise, fallback to palette class
    ...
    addMessage(data) {
        ...
        // If data.color is present, use it as inline style, else use palette class
        let userColorStyle = '';
        let userColorClass = '';
        if (data.color) {
            userColorStyle = `color: ${data.color} !important;`;
            console.debug('[DEBUG] Username', username, 'Twitch color:', data.color);
        } else {
            userColorClass = this.getUserColor(username);
            console.debug('[DEBUG] Username', username, 'Palette color class:', userColorClass);
        }
        ...
        messageElement.innerHTML = `
          ...
          <span class="username${userColorClass ? ' ' + userColorClass : ''} ${badgeClass}"${userColorStyle ? ` style=\"${userColorStyle}\"` : ''}>${username}</span>
          <button class="twitch-profile-btn" title="Ouvrir le profil Twitch" data-twitch-user="${username}">
            <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg" style="vertical-align: middle;"><rect width="20" height="20" rx="4" fill="#9147FF"/><path d="M6 5.5C5.17157 5.5 4.5 6.17157 4.5 7V13C4.5 13.8284 5.17157 14.5 6 14.5H7V16.5C7 16.7761 7.22386 17 7.5 17C7.63261 17 7.75979 16.9473 7.85355 16.8536L10.2071 14.5H14C14.8284 14.5 15.5 13.8284 15.5 13V7C15.5 6.17157 14.8284 5.5 14 5.5H6Z" fill="white"/></svg>
          </button>
          <span class="separator">:</span>
          <span class="message-content">${message}</span>
        `;

        // Ajout du handler pour ouvrir dans le navigateur par défaut
        const profileBtn = messageElement.querySelector('.twitch-profile-btn');
        if (profileBtn) {
            profileBtn.addEventListener('click', (e) => {
                e.stopPropagation();
                const user = profileBtn.getAttribute('data-twitch-user');
                const url = `https://www.twitch.tv/${encodeURIComponent(user)}`;
                if (window.require) {
                    try {
        ...
        if (this.isUserAtBottom) {
            ...
            this.newMessageWhileScrolledUp = true;
            this.showScrollBtn();
        }
    }
